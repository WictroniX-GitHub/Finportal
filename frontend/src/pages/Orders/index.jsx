...
import "../ITR_Form/ITRform.css";
import Common from "../layout/Common";
import { useNavigate } from "react-router-dom";
import Modal from "../ITR_Form/Modal";
    ...
    try {
      ...
      // else {
        try {
          ...
          await firebase.submitITR(formData, [formData.serve]).then(async() => {
              await imageUpload.forEach(async (element) => {
                await firebase.submitITRimages(element, [formData.serve])
                .then(() => 
                  navigate('/')
                )
                // console.log("working");
  ...
  return (
    <div class="full_form">
        ...
        <div class="itrform signcont">
          <form>
            <h2 class="text-black">{formData.serve}</h2>
            <Common
              imageData={imageSet}
              formData={formData}
    ...
    =               <label htmlFor="" className="titles">
                  ...
                  <div className="first-box" style={{marginLeft: "4vh"}}>
                    <label htmlFor="" className="titles">
            ...
            {formData[formData.serve].status !== "accept" &&
            ...
            <div className="preview button">
              <Preview heading={formData.serve} />
            </div>
            
            {formData.serve === filedITR && formData[formData.serve].status === "accept" &&
              <div className="preview button">
              <Modal service = {formData.serve} adminmsg={adminMSG} />
            </div>
            }
